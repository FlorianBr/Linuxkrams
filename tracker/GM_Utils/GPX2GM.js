...
// Version 5.12.1
// 13. 12. 2014 Jürgen Berkemeier
// www.j-berkemeier.de

"use strict";

window.JB = window.JB || {};
JB.GPX2GM = {ver:"5.12.1",dat:"13. 12. 2014",fname:"GPX2GM.js"};
	...
	function showTracks() {
		...
		for(var i=0;i<gpxdaten.tracks.anzahl;i++) if(chktrk.status[gpxdaten.tracks.anzahl==1?0:i+1]) {
			...
			if(JB.gc.shtrt && tracki.t0>0) 
				info += "<br />"+strings.tstart+":  <span style='white-space:nowrap;'>" + JB.sec2string(tracki.t0*3600,JB.gc.tdiff*3600) + "</span>"; 
...
JB.Map = function(mapcanvas,id) {
	...
	var osmmapde = new google.maps.ImageMapType({
		getTileUrl: function(ll, z) {
			var X = ll.x % (1 << z); if(X<0) X += (1 << z);
			return "https://c.tile.openstreetmap.de/tiles/osmde/" + z + "/" + X + "/" + ll.y + ".png";
		},
		tileSize: new google.maps.Size(256, 256),
		isPng: true,
		maxZoom: 19,
	...
	var osmcycle = new google.maps.ImageMapType({
		getTileUrl: function(ll, z) {
			var X = ll.x % (1 << z); if(X<0) X += (1 << z);
			return "https://c.tile.opencyclemap.org/cycle/" + z + "/" + X + "/" + ll.y + ".png";
		},
		tileSize: new google.maps.Size(256, 256),
		isPng: true,
		maxZoom: 19,
  ...
  var osmlandscape = new google.maps.ImageMapType({
    getTileUrl: function(ll, z) {
      var X = ll.x % (1 << z); if(X<0) X += (1 << z);
      return "https://c.tile3.opencyclemap.org/landscape/" + z + "/" + X + "/" + ll.y + ".png";
    },
    tileSize: new google.maps.Size(256, 256),
    isPng: true,
    maxZoom: 19,
	...
	google.maps.event.addListener(this.map, "maptypeid_changed", function() {
		...
		if (dieses.map.getMapTypeId() == 'osm') {
			osmcopyright.innerHTML = 'Map data &copy; <a href="https://www.openstreetmap.org/" target="_blank">OpenStreetMap</a> and contributors <a href="http://creativecommons.org/licenses/by-sa/2.0/" target="_blank">CC-BY-SA</a>';
			dieses.maxzoom = 19;
		} 
			...
			osmcopyright.innerHTML = 'Map data &copy; <a hrefs="http://www.opencyclemap.org/" target="_blank">OpenCycleMap</a> and contributors <a href="http://creativecommons.org/licenses/by-sa/2.0/" target="_blank">CC-BY-SA</a>';
			dieses.maxzoom = 18;
		} 
		else if (dieses.map.getMapTypeId() == 'landscape') {
			osmcopyright.innerHTML = 'Map data &copy; <a hrefs="http://www.opencyclemap.org/" target="_blank">OpenLandscapeMap</a> and contributors <a href="http://creativecommons.org/licenses/by-sa/2.0/" target="_blank">CC-BY-SA</a>';
			dieses.maxzoom = 18;
		} 
...
JB.lpgpx = function(fns,id,callback) { 
	...
	function parseGPX(xml,gpxdaten,id,fnr) {
		...
		for(var k=0;k<trk.length;k++) { 
			...
			if(trkptslen>1) {
				tnr++; 
				var tracki = { laenge:0, rauf:0, runter:0, t0:0, vmitt:0, vmittwop:0, fnr:fnr};
...
JB.GPX2GM.start = function() {
	...
	JB.LoadScript("https://maps.google.com/maps/api/js?sensor=false&libraries=geometry&callback=JB.gmcb", function() {});
